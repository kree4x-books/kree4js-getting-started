...
// 服务拦截器做"服务认证 + 结果脱敏"：
//   - 调用发起者：出站拦截器自动注入 Token，业务代码零感知
//   - 被调用者：入站拦截器校验 Token，非法直接短路拒绝
//   - 被调用者：入站 afterCall 统一脱敏，在结果回传调用者前改写 value
// options 会随服务调用跨网传送，入站拦截器与服务方法 ctx 都能取到。

// ── 出站拦截器（挂在调用发起者）：自动注入 Token ────────
class TokenInjectInterceptor {
  beforeCall (ctx, cluster, method, params, options) {
    // options 随调用序列化跨网传送，在这里"搭车"注入 Token
    options.token = TOKEN
...
async function main () {
  ...
  nodeA.register('data', {
    getUser (id, ctx) {
      // 服务方法的最后一个参数是 ServiceCallContext，
      // ctx.serviceCall.options.token 可取得调用发起者注入的 Token
      logger.info(`[nodeA] getUser() 执行，收到 Token: ${ctx.serviceCall.options.token}`)
  ...
  try {
    ...
    // nodeB：业务代码直接调用，无感知 Token（拦截器自动注入）
    const dataB = nodeB.service('data')

    // nodeC：同样调用，未注入 Token，被入站拦截器拒绝
    const dataC = nodeC.service('data')

    logger.info('--- nodeB：合法调用，结果被 afterCall 统一脱敏 ---')
    const user = await dataB.getUser('u-1')
    logger.info(`nodeB 拿到脱敏后的数据: ${JSON.stringify(user)}`)
    ...
    try {
      ...
      logger.info('nodeC 调用被拒绝: ok=false')
    }
  } finally {
