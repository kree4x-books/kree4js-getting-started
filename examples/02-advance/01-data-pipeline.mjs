...
// 入站访问控制：黑名单中节点发出的 Call 直接丢弃（短路 + 丢弃，AND 语义）
class AccessControlPipe extends Pipe {
  ...
  process (thing) {
    // 只拦截服务调用（ServiceCall，typeCode 'C'），放行 Beacon/WhoHas/IHave 等发现信令
...
/**
 * 数据管线：使用 Pipeline 处理出站入站原始数据。
 *
 * - nodeA（监听）：注册 greet 服务；入站管线=日志+访问控制，出站管线=日志
 * - nodeB（attach）：合法客户端，正常调用
 * - nodeC（attach）：被拉黑客户端，其入站 Call 被 nodeA 丢弃 → 无响应 → 调用超时
 *
 * 关键点：
 * - 两条管线：ThingOut（发出前）/ ThingIn（收到后）
 * - AND 语义：任一 Pipe 返回 null/undefined 即短路
 * - 顺序敏感：Pipe 按注册顺序执行
...
async function main () {
  ...
  blocklist.add(nodeC.id) // 拉黑 nodeC：其入站 Call 会被 nodeA 丢弃
  ...
  try {
    ...
    // 1. nodeB 正常调用：命中服务 → 返回
    logger.info('--- 1. nodeB 正常调用 ---')
    const greetB = nodeB.service('greet')
    const r1 = await greetB.hello('World')
    logger.info(`[nodeB] greet.hello('World') => ${r1}`)

    // 2. nodeC 被拉黑：nodeA 入站 AccessControl 丢弃 → 无响应 → 调用超时
    logger.info('--- 2. nodeC 被拉黑：nodeA 入站丢弃 → 无响应 → 调用超时 ---')
