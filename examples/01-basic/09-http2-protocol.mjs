...
/**
 * HTTP/2协议连接示例：两个NodeJS节点通过HTTP/2协议互联，进行双向RPC调用。
 *
 * - nodeA（https2-listen）：创建HTTP/2服务器（TLS），监听端口，注册calc服务
 * - nodeB（https2-attach）：以HTTP/2客户端身份连接到nodeA，注册str服务
 *
 * 关键点：
 * - HTTP/2需要TLS证书，服务端使用自签证书（openssl临时生成）
 * - 客户端使用rejectUnauthorized: false跳过证书校验（仅演示用）
 * - 一条stream = 一个channel，原生多路复用，无需WebSocket升级
 *
 * 调用流程：
...
function createSelfSignedCerts () {
  ...
  // SAN配置让自签证书对localhost/127.0.0.1生效；无SAN时TLS校验证书主机名会失败
