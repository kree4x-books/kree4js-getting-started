...
/**
 * 对等节点RPC通信（Peer-to-Peer）。
 *
 * 两个对等节点通过TCP互联，各自注册identifier服务，互相调用。
 * kreex没有严格的client/server概念 —— 两个节点对称对等，
 * listen/attach只是连接建立的方向不同，连接后任意一方都可以注册和调用服务。
 */
async function main () {
  ...
  try {
    await nodeA.start()
    logger.info('[node-a] 已启动，监听tcp://127.0.0.1:8000')
    await nodeB.start()
