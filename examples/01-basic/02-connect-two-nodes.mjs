 ...
 * 两个对等节点的连接建立（仅验证连通性）。
 *
 * node-a 监听 TCP 端口，node-b 主动 attach 连入；
 * 框架提供了语法糖 whenReady()，用于等待并检查两节点之间是否建立数据连接。
 * 退出时无论连接是否成功，都需在 finally 中顺序停止两个节点，
 * 且单个节点 stop 失败不应阻塞另一个节点退出。
 */
async function main () {
  // ── Node A：监听端口 ─────────────────────────────
  ...
  try {
    ...
    // 语法糖：等待并检查两个节点间是否有已发现彼此，超时 5000ms
