...
/**
 * 间接服务调用：通过 Hub 节点转发 RPC 请求。
 *
 * 星型拓扑（Hub-Leaf）：
 * - Hub（中心节点）：hub，负责转发所有消息
 * - Leaf（外围节点）：leafNode1、leafNode2，只能与 Hub 直连
 *
 * 拓扑结构：
 *   leafNode1 → hub ← leafNode2
 *
 * 服务注册：
 *   leafNode1 注册 greet 服务
 *
 * 连接关系：
 *   leafNode1 连接到 hub
 *   leafNode2 连接到 hub
 *
 * 调用流程：
 *   leafNode2 调用 greet 服务，经 hub 转发到 leafNode1
 */
async function main () {
  // ── Hub Node（中心转发节点） ──────────────────────
  ...
  // ── Leaf Node 2（调用方，连接 Hub） ────────────────
  const leafNode2 = Kree4n.create('leaf-2')
  leafNode2.attach('tcp://127.0.0.1:8010')

  try {
    await hub.start()
    logger.info('[hub] 已启动，监听 tcp://127.0.0.1:8010')

    await leafNode1.start()
    logger.info('[leafNode1] 已启动，已连接到 hub')

    await leafNode2.start()
    logger.info('[leafNode2] 已启动，已连接到 hub')

    // leafNode2 调用 leafNode1 的 greet 服务，经 hub 转发
